...
if ($strand_id <= 0 || $grade_level <= 0 || $semester === '' || $subject_code === '' || $subject_name === '') {
...
if ($subject_id > 0) {
    ...
    $stmt->execute([$strand_id, $grade_level, $semester, $subject_code, $subject_name, $subject_id]);
} else {
    $stmt = $conn->prepare('INSERT INTO subjects (strand_id, grade_level, semester, subject_code, subject_name) VALUES (?, ?, ?, ?, ?)');
    $stmt->execute([$strand_id, $grade_level, $semester, $subject_code, $subject_name]);
}
